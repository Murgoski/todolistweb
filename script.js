        ...
        .then((willDelete) => {
            if (willDelete) {
                ...
                swal({
                    ...
                    icon: "success",
                    buttons: false, // Hides the confirm button
                    timer: 2000,
                });
            }
        });
...
document.getElementById('addTask').addEventListener('click', function() {
    ...
    document.getElementById('task_label').value = 'Select Task Label';
...
function openFormDropdown() {
    ...
    document.addEventListener("click", function(event) {
        if (!event.target.matches('.FormDropbtn') && !dropdownContent.contains(event.target)) {
            closeDropdown();
        }
    });
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function updateProgress() {
        ...
        if (progress < 100) {
            progress++;
            setTimeout(updateProgress, 15); // Adjust the speed of progress here
        } else {
            // Hide loading screen
